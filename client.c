...
int main() {
    ...
    while (1) { 
        ...
        if (strcmp(buffer, "/chatbot_v2 login") == 0) {
            chatbotv2_enabled = 1; // Enable chatbot_v2
        } else if (strcmp(buffer, "/chatbot_v2 logout") == 0) {
            chatbotv2_enabled = 0; // Disable chatbot_v2
        }
...
void *receive_message(void *socket) {
    ...
    while (1) {
        ...
        if (length > 0) {
            ...
            if(chatbotv2_enabled) {
                printf("GPTUser> "); // Display prompt after receiving a message when chatbot is enabled
                fflush(stdout);
            }
        } else if (length == 0) {
            no_ofclient++;
