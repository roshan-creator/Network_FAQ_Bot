...
int main() {
    ...
    while(1) {
        ...
        client_add->chatbot_v2_enabled = 0;
        cli->sockfd = connfd;
        cli->uid = cli_count++;
    ...
    while(1) {
        ...
        if (receive > 0) {
            if(strlen(buff_out) > 0) {
                ...
                if(strcmp(buff_out, "/chatbot_v2 login") == 0) {
                    cli->chatbot_v2_enabled = 1;
                    char login_message[] = "gpt2bot> Hi, I am updated bot, I am able to answer any question be it correct or incorrect\n";
                    send_message_self("gpt2bot> Hi, I am updated bot, I am able to answer any question be it correct or incorrect\n", cli->sockfd);
                    //fprintf(cli->chat_history, "%s", login_message); // Log chatbot login message
                    continue;
                }

                // Chatbot logout command
                if(strcmp(buff_out, "/chatbot_v2 logout") == 0) {
                    cli->chatbot_v2_enabled = 0;
                    char logout_message[] = "gpt2bot> Bye! Have a nice day and do not have any complain about me\n";
                    send_message_self("gpt2bot> Bye! Have a nice day and do not have any complain about me\n", cli->sockfd);
                    //fprintf(cli->chat_history, "%s", logout_message); // Log chatbot logout message
                    continue;
                }
                ...
                if(cli->chatbot_enabled) {
                    ...
                    if (answer != NULL) {
                        ...
                        snprintf(response, BUFFER_SIZE, "stupidbot> %s\n", answer);
                        
                        // Send the response.
                        send_message_self(response, cli->sockfd);
                        
                        // After sending, continue to the next iteration of the loop to avoid further processing.
                        continue;
                    }
                ...
                if (cli->chatbot_v2_enabled) {
                    char cmd[9000];
                    snprintf(cmd, sizeof(cmd), "python3 gpt2_chatbot.py \"%s\"", buff_out);
                    FILE* python_script = popen(cmd, "r");
                    if (python_script) {
                        char response[BUFFER_SIZE];
                        fgets(response, BUFFER_SIZE, python_script);
                        pclose(python_script);
                        //snprintf(buff_out, BUFFER_SIZE, "gpt2bot> %s\n", response);
                        // Ensure the response is properly null-terminated.
                        response[strcspn(response, "\n")] = 0; // Remove newline character, if present.

                        // Format the response with "gpt2bot>" prefix.
                        size_t formatted_response_size = snprintf(NULL, 0, "gpt2bot>%s\n", response) + 1; // +1 for null terminator

                        // Dynamically allocate the buffer
                        char *formatted_response = malloc(formatted_response_size);

                        snprintf(formatted_response, formatted_response_size, "gpt2bot>%s\n", response);

                        // Send the formatted response back to the client.
                        send_message_self(formatted_response, cli->sockfd);
